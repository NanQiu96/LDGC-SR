...
parser.add_argument('--dataset', default='sample', help='dataset name: diginetica/retailrocket/yoochoose/sample')
    ...
    for data in reader:
        ...
        if curdate and not curid == sessid:
            ...
            if opt.dataset == 'yoochoose':
                ...
                date = curdate
                if minDate == -1 or date < minDate:
                    minDate = date
                if maxDate == -1 or date > maxDate:
                    maxDate = date
            else:
        ...
        if opt.dataset == 'yoochoose':
            ...
            curdate = int(data['timestamp']) // 1000
        else:
...
for s in list(sess_clicks):
    ...
    if len(filseq) < 2 or (opt.dataset == 'retailrocket' and len(filseq) > 50):
        del sess_clicks[s]
        del sess_date[s]
    else:
...
for seq in tra_seqs:
    all += len(seq)
print('train avg length: ', all/(len(tra_seqs) * 1.0))
for seq in tes_seqs:
    all += len(seq)
print("clicks: ", all)
