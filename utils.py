...
def handle_adj_items(inputs, adj, last_len):
    last_items, adj_items = [], []
    for u_input in inputs:
        if len(u_input) < last_len:
            u_input = u_input * (last_len // len(u_input) + 1)
        items = u_input[-last_len: ]
        last_items.append(items)
        adj_item = []
        for item in items:
            adj_item += adj[item].tolist()
        adj_items.append(adj_item)
    return last_items, adj_items


class Data(Dataset):
    def __init__(self, data, adj, last_len=2, train_len=None):
        last_items, adj_items = handle_adj_items(data[0], adj, last_len)
        inputs, mask, max_len = handle_data(data[0], train_len)
        self.last_items = np.asarray(last_items)
        self.adj_items = np.asarray(adj_items)
    ...
    def __getitem__(self, index):
        ...
        last_item, adj_item = self.last_items[index], self.adj_items[index]
        ...
        return [torch.tensor(alias_inputs), torch.tensor(adj), torch.tensor(items),
                ...
                torch.tensor(last_item), torch.tensor(adj_item)]
